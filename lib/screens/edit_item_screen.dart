...
class _EditItemScreenState extends State<EditItemScreen> {
  ...
  void initState() {
    ...
    _tanggalMasuk = DateTime.parse(widget.item.tanggalMasuk);
    _tanggalKedaluwarsa = DateTime.parse(widget.item.tanggalKedaluwarsa);
  }
  ...
  Future<void> _selectDate(BuildContext context, bool isTanggalMasuk) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: isTanggalMasuk ? _tanggalMasuk! : _tanggalKedaluwarsa!,
  ...
  Future<void> _handleSubmit() async {
    if (_formKey.currentState!.validate()) {
      ...
      bool success = await itemProvider.updateItem(widget.item.id, updatedItem);
      ...
      if (success && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              itemProvider.errorMessage ?? 'Gagal memperbarui item',
            ),
            backgroundColor: Colors.red,
          ),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Edit Inventaris Fathan'), centerTitle: true),
